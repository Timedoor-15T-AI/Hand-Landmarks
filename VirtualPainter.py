...
while True:
  ...
  frameGray = cv2.cvtColor(imgCanvas, cv2.COLOR_BGR2GRAY) # Convert to Gray
  _, frameInvers = cv2.threshold(frameGray, 50, 255, cv2.THRESH_BINARY_INV) # Invert
  frameInvers = cv2.cvtColor(frameInvers, cv2.COLOR_GRAY2BGR) # Convert to BGR
  frame = cv2.bitwise_and(frame, frameInvers) # Use Inverse as a Mask
  frame = cv2.bitwise_or(frame, imgCanvas) # Use Canvas as a Mask
